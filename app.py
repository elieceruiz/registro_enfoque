import streamlit as st import pandas as pd import time import os import json from datetime import datetime, timedelta import plotly.express as px

st.set_page_config(page_title="Gestor de Enfoque y Pausa", layout="centered") st.title("Gestor de Enfoque y Pausa Activa")

archivo_csv = "registro_enfoque.csv" archivo_json = "registro_enfoque.json"

Cargar historial si existen

if os.path.exists(archivo_csv): df_hist = pd.read_csv(archivo_csv) else: df_hist = pd.DataFrame(columns=["Actividad", "Inicio", "Fin", "Duración", "Estado"]) df_hist.to_csv(archivo_csv, index=False)
...
with tab1: unidad = st.radio("¿Trabajar en minutos o segundos?", ["minutos", "segundos"]) factor = 60 if unidad == "minutos" else 1
...
if st.button("Iniciar sesión de enfoque") or st.session_state.get("iniciar_con_enter"):
    ...
    for i in range(tiempo_total):
        ...
        tiempo_restante = tiempo_total - (i + 1)
        texto.text(f"Enfoque: {i + 1}/{tiempo_total} segundos - Restan {tiempo_restante}s")
    ...
    if st.checkbox("¿Tomar pausa activa ahora?"):
        ...
        barra_pausa = st.empty()
        texto_pausa = st.empty()
        inicio_pausa = datetime.now()
        for i in range(tiempo_total_pausa):
            time.sleep(1)
            porcentaje = (i + 1) / tiempo_total_pausa
            tiempo_restante = tiempo_total_pausa - (i + 1)
            texto_pausa.text(f"Pausa: {i + 1}/{tiempo_total_pausa} segundos - Restan {tiempo_restante}s")
...
with tab2: st.subheader("Historial de sesiones")
